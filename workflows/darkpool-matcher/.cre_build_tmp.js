...
class BaseError4 extends Error {
  constructor(shortMessage, args = {}) {
    const details = args.cause instanceof BaseError4 ? args.cause.details : args.cause?.message ? args.cause.message : args.details;
    const docsPath3 = args.cause instanceof BaseError4 ? args.cause.docsPath || args.docsPath : args.docsPath;
    const message = [
      shortMessage || "An error occurred.",
      "",
      ...args.metaMessages ? [...args.metaMessages, ""] : [],
...
class UnknownTypeError3 extends BaseError4 {
  constructor({ type }) {
    super("Unknown type.", {
      metaMessages: [
...
class UnknownSolidityTypeError2 extends BaseError4 {
  constructor({ type }) {
    super("Unknown type.", {
...
class InvalidParameterError3 extends BaseError4 {
  constructor({ param }) {
...
class SolidityProtectedKeywordError3 extends BaseError4 {
...
class InvalidModifierError3 extends BaseError4 {
...
class InvalidFunctionModifierError3 extends BaseError4 {
  constructor({ param, type, modifier }) {
    super("Invalid ABI parameter.", {
      details: param,
      metaMessages: [
...
class InvalidAbiTypeParameterError3 extends BaseError4 {
...
class InvalidSignatureError3 extends BaseError4 {
...
class UnknownSignatureError3 extends BaseError4 {
...
class InvalidStructSignatureError3 extends BaseError4 {
  constructor({ signature }) {
...
class CircularReferenceError3 extends BaseError4 {
  constructor({ type }) {
...
class InvalidParenthesisError3 extends BaseError4 {
...
function parseFunctionSignature(signature, structs = {}) {
  ...
  for (let i2 = 0;i2 < inputLength; i2++) {
    inputs.push(parseAbiParameter3(inputParams[i2], {
  ...
  if (match.returns) {
    ...
    for (let i2 = 0;i2 < outputLength; i2++) {
      outputs.push(parseAbiParameter3(outputParams[i2], {
        modifiers: functionModifiers,
        structs,
        type: "function"
      }));
...
function parseEventSignature(signature, structs = {}) {
  ...
  for (let i2 = 0;i2 < length; i2++)
    abiParameters.push(parseAbiParameter3(params[i2], {
...
function parseConstructorSignature(signature, structs = {}) {
  ...
  const length = params.length;
  for (let i2 = 0;i2 < length; i2++)
    abiParameters.push(parseAbiParameter3(params[i2], { structs, type: "constructor" }));
  return {
    type: "constructor",
    stateMutability: match.stateMutability ?? "nonpayable",
...
function parseAbiParameter3(param, options) {
  ...
  if (isTuple) {
    ...
    const length = params.length;
    for (let i2 = 0;i2 < length; i2++) {
      components_.push(parseAbiParameter3(params[i2], { structs }));
    }
...
function parseAbi3(signatures) {
  ...
  for (let i2 = 0;i2 < length; i2++) {
    ...
    if (isStructSignature(signature))
      continue;
    abi.push(parseSignature3(signature, structs));
  }
  return abi;
}
var AggregatorV3InterfaceAbi = parseAbi3([
  "function latestRoundData() external view returns (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound)",
  "function decimals() external view returns (uint8)"
]);
var ZKDarkPoolAbi = parseAbi3([
...
var configSchema = exports_external.object({
  ...
  darkPoolAddress: exports_external.string(),
  receiverAddress: exports_external.string(),
  forwarderAddress: exports_external.string(),
  gasLimit: exports_external.string()
});
var ORACLE_PRECISION_MULTIPLIER = 10n ** 10n;
function getEvmClient(runtime2) {
  const network295 = getNetwork({
    chainFamily: "evm",
  ...
  return new ClientCapability(network295.chainSelector.selector);
}
function readEthUsdPrice(runtime2) {
  const evmClient = getEvmClient(runtime2);
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: runtime2.config.priceFeedAddress,
      data: callData
    }),
  ...
  runtime2.log(`ETH/USD = ${price18.toString()} (1e18) at round ${roundId}, updated ${updatedAt}`);
  return price18;
}
function queryCheckUpkeep(runtime2) {
  const evmClient = getEvmClient(runtime2);
  const callData = encodeFunctionData({
    abi: ZKDarkPoolAbi,
  ...
  const contractCall = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: runtime2.config.darkPoolAddress,
      data: callData
    }),
    blockNumber: LAST_FINALIZED_BLOCK_NUMBER
  }).result();
  ...
  runtime2.log(`checkUpkeep => upkeepNeeded=${upkeepNeeded}, performData=${bytesToHex3(performData)}`);
  return { upkeepNeeded, performData };
}
function submitPerformUpkeep(runtime2, performData) {
  const evmClient = getEvmClient(runtime2);
  const callData = encodeFunctionData({
    abi: ZKDarkPoolAbi,
    functionName: "performUpkeep",
    args: [bytesToHex3(performData)]
  });
  ...
  runtime2.log(`performUpkeep submitted: https://sepolia.etherscan.io/tx/${txHash}`);
  return txHash;
}
var onCronTrigger = (runtime2) => {
  const price18 = readEthUsdPrice(runtime2);
  const { upkeepNeeded, performData } = queryCheckUpkeep(runtime2);
  if (!upkeepNeeded) {
    runtime2.log("No matchable dark pool orders at current price; skipping.");
    return { price18: price18.toString(), upkeepNeeded: false };
  }
  const txHash = submitPerformUpkeep(runtime2, performData);
